...
def depth_first_graph_search(problem):
    ...
    while frontier:
        # Increment the nodes traversed count
        nodes_traversed += 1
        
        # Visualize the current grid state
        visualize_sudoku_grid(frontier[-1].state, None)
        pg.time.delay(50)  # Adjust the delay as needed
        ...
        for child in node.expand(problem):
            if tuple(map(tuple, child.state)) not in explored and child not in frontier:
                ...
                # Print the number being tried
                visualize_sudoku_grid(child.state, f"Trying {child.action[2]}")
            else:
                # Increment the backtracked count
                backtracked_count += 1
    
    # Print the total number of times backtracked and nodes traversed
    print(f"Total number of times backtracked: {backtracked_count}")
    print(f"Total number of nodes traversed: {nodes_traversed}")
...
def visualize_sudoku_grid(grid, action):
    ...
    for i in range(9):
        for j in range(9):
            rect = pg.Rect(j * cell_size + gap, i * cell_size + gap, cell_size - gap, cell_size - gap)
            pg.draw.rect(screen, (0, 0, 0), rect, 2)
            if grid[i][j] != "":
                text_surface = font.render(grid[i][j], True, (0, 0, 0))
                screen.blit(text_surface, (j * cell_size + cell_size // 2 - text_surface.get_width() // 2, i * cell_size + cell_size // 2 - text_surface.get_height() // 2))

    # Print the action (number being tried or backtracking)
    if action:
        text_surface = font.render(action, True, (255, 0, 0))
...
def solve_and_visualize(initial_state):
    problem = Problem(initial_state)
    solution_node = depth_first_graph_search(problem)
...
draw_menu()
menu_loop()
initial_state = generate_sudoku()
solve_and_visualize(initial_state)
